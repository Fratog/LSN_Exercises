...
Chromosome::Chromosome(int N, int start, Random & rnd) : Ngenes(N) //N is the cities number 0 would be the index of the firs city, i.e. the first element of the cities matrizx of Ncities, in general start is the index of the home city
{
	vector<int> allele;
	int appo=0;
...
void Chromosome::calculate_fit(const vector<vector<double>> & cities) //matrix of citites coordinates
{
	...
	//cout<<"check size	"<<Chrom.size()<<endl;	
  //cout<<"calculate_fit"<<fit<<endl;
}
...
void Chromosome::mutation1(Random & rnd)
{
	int i=rnd.Rannyu(1, Ngenes-1); //gives [1, Ngenes-2]. 
	std::swap(Chrom[i], Chrom[i+1]); 
}
...
void Chromosome::mutation2(Random & rnd)
{
  ...
  std::rotate(Chrom.begin() + i, Chrom.begin() + i + m, Chrom.begin() + j);//	
}
...
//oss se i e j sono tali che, i=j o j=i+1 allora non fa niente, ma secondo me va bene così
void Chromosome::mutation4(Random & rnd) 
{
	...
	if(i<j)	
		std::reverse(Chrom.begin()	+	i, Chrom.begin() + j);
	else
		std::reverse(Chrom.begin()	+	j, Chrom.begin() + i);
}
...
/*
void Chromosome::crossover(Chromosome & Chrom2, Random & rnd) //don't use this one use the one in popullation, and any_of method is better than the other one
{
